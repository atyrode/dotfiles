...
<!--
  Vendored verbatim from ogulcancelik/herdr SKILL.md at tag v0.7.4 and
  reviewed before landing (#269). Upstream skill text becomes agent
  instructions, so this copy is refreshed ONLY by reviewing the upstream
  diff — never automatically. checks/herdr.nix fails when the marker below
  lags the pkgs/herdr pin, and scripts/update-pins.sh prints the review
  pointer alongside a herdr bump.
  HERDR_SKILL_UPSTREAM_VERSION=0.7.4
-->
