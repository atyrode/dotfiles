...
usage() {
  cat <<'EOF'
Usage:
  ./bootstrap/install.sh preflight [OPTIONS]
  ./bootstrap/install.sh plan [OPTIONS]
  ./bootstrap/install.sh apply [OPTIONS]
  ./bootstrap/install.sh recover [OPTIONS]
  ./bootstrap/install.sh verify [OPTIONS]
...
verify_checkout() {
  ...
  git -C "$DOTFILES_DIR" ls-files --error-unmatch -- flake.nix bootstrap/install.sh >/dev/null 2>&1 ||
...
warn_if_interrupted() {
  ...
  printf '%s %s %s\n' "$(paint_err 33 'bootstrap: warning:')" \
    ...
    "$(paint_err '1;36' "./bootstrap/install.sh apply --config ${config:-$FLAKE_CONFIG}")" >&2
}
...
# The fleet binary cache is declared once, in fleet/system-boundary.json,
...
fleet_cache_setting() {
  ...
  sed -n '/"fleetCache"[[:space:]]*:/,/}/ s/^[[:space:]]*"'"$key"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*$/\1/p' \
    "$DOTFILES_DIR/fleet/system-boundary.json" 2>/dev/null
}
...
detect_fleet_cache_enrolment() {
  ...
  [[ -n "$substituter" ]] || die "fleet/system-boundary.json names no fleet cache substituter"
...
report_managed_failure() {
  ...
  if [[ "${#BROKEN_TRUST_ANCHORS[@]}" -eq 0 ]]; then
    ...
    fail BOOT-E399 \
      ...
      "send the log below so this state can get its own code and repair, or reset this machine's Nix installation with: ./bootstrap/install.sh recover --config $FLAKE_CONFIG"
    return 1
  fi
...
restart_after_source_update() {
  ...
  show_command bash "$DOTFILES_DIR/bootstrap/install.sh" "${args[@]}"
  exec bash "$DOTFILES_DIR/bootstrap/install.sh" "${args[@]}"
}
