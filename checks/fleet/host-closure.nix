...
let
  ...
  # and WSL hosts are intentionally unbudgeted (fleet/host-budgets.json).
  ...
  measure =
    ...
    assert lib.assertMsg (packageCount <= budget.maxTopLevelPackages)
      "host ${name} installs ${toString packageCount} top-level packages, over its budget of ${toString budget.maxTopLevelPackages} (fleet/host-budgets.json)";
    {
  ...
  enforce = m: ''
    ...
    if [ "$nar_bytes" -gt ${toString m.budget.maxNarBytes} ]; then
      echo "host ${m.name} closure $nar_bytes exceeds its budget ${toString m.budget.maxNarBytes} (fleet/host-budgets.json)" >&2
      failed=1
    fi
    if [ "$store_paths" -gt ${toString m.budget.maxStorePaths} ]; then
      echo "host ${m.name} store path count $store_paths exceeds its budget ${toString m.budget.maxStorePaths} (fleet/host-budgets.json)" >&2
      failed=1
    fi
