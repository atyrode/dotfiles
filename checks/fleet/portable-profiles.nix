...
assert lib.assertMsg (
  serverConfig.xdg.configFile."git/allowed_signers".source == ../../modules/home/git/allowed-signers
...
assert lib.assertMsg (
  serverConfig.xdg.configFile."git/hooks/pre-push".source == ../../modules/home/git/pre-push
  ...
  pkgs.runCommand "check-portable-profiles-${system}"
    ...
    ''
      atyrode capabilities show ${externalFixture.hostId} --json | jq -e '
        ...
        and (.capabilities | map(.name)) == ${builtins.toJSON (sort serverPolicy.capabilities)}
