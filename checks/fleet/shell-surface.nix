...
assert lib.assertMsg (!baseline.programs.fzf.enableZshIntegration)
  "fzf's unguarded zsh hook must stay disabled; modules/home/zsh/default.nix sources it behind a TTY guard (#255)";
...
pkgs.runCommand "check-shell-surface"
  ...
  ''
    ...
    zsh -dfc 'source ${../../modules/home/zsh/startup.zsh}; [[ ! -e "$HOME/local-loaded" ]]'
    zsh -dfi -c 'source ${../../modules/home/zsh/startup.zsh}; [[ -e "$HOME/local-loaded" ]]' </dev/null
    ...
    OSC_DIR="$osc_dir" zsh -dfc \
      'source ${../../modules/home/zsh/cwd.zsh}; cd -- "$OSC_DIR"; HOST=osc7-test; _atyrode_report_cwd' \
      > "$TMPDIR/osc-actual"
