...
# Regression tests for the comparison logic of ci/docs-drift-guard.sh
...
pkgs.runCommand "check-docs-drift-guard"
  {
    ...
    guard = ../../ci/docs-drift-guard.sh;
    ciInventory = ../../ci/ci.json;
  }
  ''
    # The guard resolves ci.json beside its own location; replicate the
    # repository layout.
    mkdir ci
    cp "$guard" ci/docs-drift-guard.sh
    cp "$ciInventory" ci/ci.json
    guard=$PWD/ci/docs-drift-guard.sh
