...
for i in "${!SPACE_ICONS[@]}"; do
  ...
  # DEVIATION from e6288b3: space_eager subscription. Keyboard switches
  # announce their destination before the macOS switch animation commits.
  sketchybar --add space space.$sid left \
    --set space.$sid "${space[@]}" \
    --subscribe space.$sid mouse.clicked space_eager
done
