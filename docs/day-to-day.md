...
Software worth having a few times a year is launched, not declared: `comma`
(enabled in `base`) runs any nixpkgs attribute without installing it, as in
`, difftastic`, and the next `atyrode clean` reclaims it. The names worth
remembering:

- `bpftrace` — trace what the kernel is doing to a process without building a probe (Linux only)
- `difftastic` — diff two files by syntax when a line diff is unreadable
- `graphviz` — render a dot graph to an image
- `hyperfine` — benchmark two commands against each other with real statistics
- `imagemagick` — convert, crop, or resize an image from the shell
- `pandoc` — convert a document between markup formats
- `powertop` — find out what is draining a laptop battery (Linux only)
- `qrencode` — turn a string into a QR code a phone can read
- `tcpdump` — capture packets when a service is silent and the logs are not
- `tokei` — count the lines of a tree by language
- `yt-dlp` — download a video or its audio track for offline use
