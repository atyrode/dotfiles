...
let
  ...
  packageVersion =
    ...
    in
    if version == "" || version == packageName package then "unknown" else version;
  packageDescription =
    package:
    let
    ...
    if builtins.isString description && description != "" then
      ...
      "${packageName package} package (upstream metadata unavailable)";
  packageHomepage =
    package:
    let
      homepage = package.meta.homepage or null;
    in
    if builtins.isString homepage then
      homepage
    else if builtins.isList homepage && homepage != [ ] then
  ...
  packageRecord =
    ...
    assert lib.assertMsg (
      ...
      || (builtins.isString (package.meta.description or "") && (package.meta.description or "") != "")
  ...
  packageMap =
    packages:
    builtins.listToAttrs (map (package: lib.nameValuePair (packageName package) package) packages);
  ...
  capabilityPlatforms =
    name:
    annotations.capabilities.${name}.platforms or [
      "darwin"
      "linux"
    ];
  ...
  configForCapability =
    ...
    if name == "base" then
      ...
      mkEvaluation [
        ...
        capabilityModules.${name}
      ];
  packageNamesForCapability =
    name:
    if !capabilitySupported name then
      [ ]
    else
      lib.subtractLists baseNames (sortedNames (configForCapability name).config.home.packages)
      ++ lib.optionals (name == "base") (lib.subtractLists baselineNames baseNames);
  packagesForCapability =
    name:
    if !capabilitySupported name then
      [ ]
    else
      let
        packages = packageMap (configForCapability name).config.home.packages;
      in
      map (packageName: packageRecord packages.${packageName}) (
  ...
  evaluatedCasks = lib.unique (
    map caskName (
      lib.concatMap (name: darwinConfigs.${name}.config.homebrew.casks or [ ]) (
  ...
  capabilities = lib.genAttrs capabilityNames (
    name:
  ...
  duplicateNames =
    ...
    in
    lib.filter (name: builtins.length (lib.filter (other: other == name) names) > 1) (lib.unique names);
  hostManifest =
    name: host:
    let
      selectedCapabilities = host.capabilities;
      entries = ownerEntries (
        lib.filterAttrs (capability: _: builtins.elem capability selectedCapabilities) capabilities
      );
      actualPackages = lib.subtractLists baselineNames (
        sortedNames homeConfigs.${name}.config.home.packages
      );
      attributedPackages = lib.sort builtins.lessThan (
        lib.unique (
      ...
      actualCasks =
        if platform == "darwin" then
          lib.sort builtins.lessThan (lib.unique (map caskName darwinConfigs.${name}.config.homebrew.casks))
        else
          [ ];
      attributedCasks = lib.sort builtins.lessThan (
        map (item: item.name) (
          lib.filter (
    ...
    assert lib.assertMsg (
      actualPackages == attributedPackages
    ) "inventory package attribution is incomplete for host ${name}";
    assert lib.assertMsg (
      actualCasks == attributedCasks
  ...
  unknownAnnotationKeys = lib.concatMap (
    name:
    map (key: "${name}.${key}") (
      lib.subtractLists allowedAnnotationKeys (builtins.attrNames annotations.capabilities.${name})
    )
  ) annotationNames;
