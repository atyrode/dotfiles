...
# 2026-07 extraction. This package starts from the last accepted standalone
# release as a temporary continuity bridge. Routine update automation must not
# advance it; the Manifold transition may coordinate a reviewed pin and wrapper
# update when a new plugin stage requires a newer CLI contract. The archives
# carry static Go builds (CGO disabled), so no loader fixup is needed.
let
  version = "0.19.0";
  sources = {
    "x86_64-linux" = {
